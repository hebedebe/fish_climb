...
## Sets continuous_cd
@export var continuous_cd := RigidBody2D.CCDMode.CCD_MODE_DISABLED:
	set (value):
		if continuous_cd == value:
			continuous_cd = value
			return
		continuous_cd = value
		for body in get_rigid_bodies():
			if "continuous_cd" in body.rigidbody:
				body.rigidbody.continuous_cd = continuous_cd
	get:
...
func _create_rigid_body(skeleton: Skeleton2D, bone: Bone2D, mass, is_center: bool, shape: Shape2D):
	...
	rigid_body.continuous_cd = continuous_cd
	for exclude in exclude_array:
...
## pivot is world position
func apply_torque_impulse(torque: float, pivot: Vector2) -> void:
	for softbody_el in get_rigid_bodies():
		var rigidbody := softbody_el.rigidbody
		if rigidbody is RigidBody2D:
			var angle = rigidbody.global_position.angle_to_point(pivot) + PI/2.0
			var direction = Vector2(cos(angle), sin(angle))
			(rigidbody as RigidBody2D).apply_central_impulse(direction * torque)
			
## pivot is world position
func apply_torque_force(torque: float, pivot: Vector2) -> void:
	for softbody_el in get_rigid_bodies():
		var rigidbody := softbody_el.rigidbody
		if rigidbody is RigidBody2D:
			var angle = rigidbody.global_position.angle_to_point(pivot) + PI/2.0
			var direction = Vector2(cos(angle), sin(angle))
			(rigidbody as RigidBody2D).apply_central_force(direction * torque)
